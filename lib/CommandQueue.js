...
const logger = new Logger('LPZ Client CommandQueue');
...
class CommandQueue
{
	...
	async push(command)
	{
		logger.debug('push, [command:%o]', command);
	...
	async _next()
	{
		...
		logger.debug('_next, [command:%o]', command);
	...
	{
		logger.debug('_handleCommand, [command:%o]', command);
		if (this._closed)
		{
		...
		try
		{
			...
			logger.debug('_handleCommand, [result:%o]', result);
