...
const logger = new Logger('LPZ Client Transport');
...
class Transport extends EnhancedEventEmitter
{
	...
	)
	{
		...
		logger.debug('constructor() [direction:%s, id:%s, iceParameters:%o, ' +
			'iceCandidates:%o, dtlsParameters:%o, iceServers:%o, iceTransportPolicy:%o, proprietaryConstraints:%o, '+
			' appData:%o, Handler:%o, extendedRtpCapabilities:%o, canProduceByKind:%o]',
			direction, id, iceParameters, iceCandidates, dtlsParameters, iceServers, iceTransportPolicy, proprietaryConstraints,
			appData, Handler, extendedRtpCapabilities, canProduceByKind);
		...
		logger.debug('constructor() [this._handler:%o', this._handler);
	...
	get id()
	{
		logger.debug('get id, [this._id:%o]', this._id);
		return this._id;
	}
	...
	get closed()
	{
		logger.debug('get closed, [this.closed:%o]', this._closed);
		return this._closed;
	}
	...
	get direction()
	{
		logger.debug('get direction, [this._direction:%o]', this._direction);
	...
	get handler()
	{
		logger.debug('get handler, [this._handler:%o]', this._handler);
	...
	get connectionState()
	{
		logger.debug('get connectionState, [this._connectionState:%o]', this._connectionState);
	...
	async getStats()
	{
		...
		logger.debug('getStats, [this._handler.getTransportStats:%o]', this._handler.getTransportStats());
	...
	{
		logger.debug('restartIce(), [iceParameters:%o]', iceParameters);
	...
	{
		logger.debug('updateIceServers(), [iceServers:%o]', iceServers);
	...
	{
		logger.debug('produce() [track:%o, encodings:%o, codecOptions:%o, appData:%o]', track, encodings, codecOptions, appData);
		...
		return this._awaitQueue.push(
			async () =>
			{
				...
				logger.debug('produce() [localId:%o, rtpParameters:%o]', localId, rtpParameters);
				try
				{
	...
	{
		logger.debug('consume(), [id:%o, producerId:%o, kind:%o, rtpParameters:%o, appData:%o]', id, producerId, kind, rtpParameters, appData);
		...
		return this._awaitQueue.push(
			async () =>
			{
				logger.debug('consume(), [rtpParameters:%o, _extendedRtpCapabilities:%o]', rtpParameters, this._extendedRtpCapabilities);
				// Ensure the device can consume it.
				const canConsume = ortc.canReceive(
					rtpParameters, this._extendedRtpCapabilities);

				logger.debug('consume(), [canConsume:%o]', canConsume);
				if (!canConsume)
				...
				logger.debug('consume(), [localId:%o, track:%o]', localId, track);
				const consumer =
					new Consumer({ id, localId, producerId, track, rtpParameters, appData });

				logger.debug('consume(), [consumer:%o]', consumer);
	...
	_handleHandler()
	{
		...
		logger.debug('_handleHandler, [handler:%o]', handler);
		handler.on('@connect', ({ dtlsParameters }, callback, errback) =>
		{
			logger.debug('_handleHandler, connect [dtlsParameters:%o, handler:%o]', dtlsParameters, handler);
			if (this._closed)
			{
			...
			logger.debug('_handleHandler, connect 2');
			this.safeEmit('connect', { dtlsParameters }, callback, errback);
		});

		handler.on('@connectionstatechange', (connectionState) =>
		{
			logger.debug('_handleHandler, connectionstatechange [connectionstatechange:%o, handler:%o]', connectionState, handler);
	...
	{
		logger.debug('_handleProducer, [producer:%o]', producer);
		producer.on('@close', () =>
		{
			logger.debug('_handleProducer, close [producer:%o]', producer);
		...
		{
			logger.debug('_handleProducer, replacetrack [producer:%o, track:%o]', producer, track);
			this._awaitQueue.push(
		...
		{
			logger.debug('_handleProducer, setmaxspatiallayer [producer:%o, spatialLayer:%o]', producer, spatialLayer);
			this._awaitQueue.push(
				async () => (
		...
		{
			logger.debug('_handleProducer, getstats [producer:%o]', producer);
			if (this._closed)
	...
	{
		logger.debug('_handleConsumer, [consumer:%o]', consumer);
		consumer.on('@close', () =>
		{
			logger.debug('_handleConsumer, close [consumer:%o]', consumer);
		...
		{
			logger.debug('_handleConsumer, getstats [consumer:%o]', consumer);
			if (this._closed)
				return errback(new InvalidStateError('closed'));
